...
export function CardMatchGame({ pairs, status, isLoading, onComplete, onAttempt }: CardMatchGameProps) {
  ...
  if (isLoading || status === 'PENDING') {
    return (
      <div className="mx-auto w-full max-w-[820px] rounded-3xl border border-gray-100 bg-white/60 p-6 shadow-sm">
  ...
  if (!pairs.length || status === 'FAILED') {
    return (
      <div className="mx-auto w-full max-w-[820px] rounded-3xl border border-gray-100 bg-white/60 p-6 text-center text-sm text-gray-400">
        카드 매칭 데이터를 준비 중입니다.
      </div>
    )
  }
  ...
  return (
    <div className="mx-auto w-full max-w-[820px] rounded-3xl bg-white/70 p-6">
      <div className="grid grid-cols-4 gap-4">
        {cards.map(card => {
